...
const backup = 'c:\\Users\\hrjeong\\Desktop\\광명 스마트데이터포털\\src\\components\\MileDashboard.tsx.backup';
...
content = content.replace(
  /interface MileDashboardProps \{\s*onShowToast: \(msg: string\) => void;\s*\}/,
...
content = content.replace(
  /export default function MileDashboard\(\{ onShowToast \}: MileDashboardProps\) \{[\s\S]*?const tabs = \[/,
  `export default function MileDashboard({ onShowToast, activeTab: externalActiveTab, onTabChange, title, subtitle, customTabs }: MileDashboardProps) {
...
// We replace the specific array inside the JSX mapping
content = content.replace(
  /\[\s*\{\s*label:\s*'광명스카이돔 태양광'[\s\S]*?\]\.map/, 
  'currentData.chartData.map'
);

// Fix arithmetic error in chart
content = content.replace(/const h = \(item\.val \/ 250\) \* 160;/, 'const h = (Number(item.val) / 250) * 160;');

// 5. Make Table Title dynamic
content = content.replace(/<span className="ttl">광명역 태양광 발전량 데이터<\/span>/g, '<span className="ttl">{currentData.tableTitle}</span>');

// 6. Make the rows dynamic
content = content.replace(/<td className="text-left"><span className="ttl">광명역 태양광 발전량 데이터<\/span><\/td>/g, '<td className="text-left"><span className="ttl">{currentData.tableTitle} 상세 {99 - idx}</span></td>');

// 7. Inject tabData at the top (AFTER we did the chart replacement, so it doesn't match our tabData!)
const tabDataStr = `
...
content = content.replace(/(const tabs = .*?\];)/s, '$1\n' + tabDataStr);
...
console.log('Fully restored and dynamicized!');
